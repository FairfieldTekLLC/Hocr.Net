...
internal class PdfCreator : IDisposable
{
    ...
    private void AddImage(iTextSharp.text.Image image, Rectangle dimension)
    {
        ...
        */
        ...
        Rectangle newDim = new(dwidth * 72.0f, dHeight * 72.0f);
    ...
    public string AddPage(Image image, PdfMode mode, string sessionName, Rectangle dimension)
    {
        ...
        FrameDimension frameDim = new(objGuid);
                ...
                case PdfMode.DrawBlocks:
                    {
                        _ocrController.AddToDocument(PdfSettings.Language, image, ref _hDoc, sessionName);
                        HPage page = _hDoc.Pages[_hDoc.Pages.Count - 1];
                        WritePageDrawBlocks(image, page, dimension);
                        pageBody = pageBody + page.TextUnescaped;
                        break;
                    }
    ...
    private iTextSharp.text.Image GetImageForPdf(Image image)
    {
        ...
        switch (PdfSettings.ImageType)
        {
            case PdfImageType.Png:
                img = ImageProcessor.ConvertToImage(image, "PNG", PdfSettings.ImageQuality, PdfSettings.Dpi);
                break;
            case PdfImageType.Jpg:
                img = ImageProcessor.ConvertToImage(image, "JPEG", PdfSettings.ImageQuality, PdfSettings.Dpi);
                break;
            case PdfImageType.Bmp:
                img = ImageProcessor.ConvertToImage(image, "BMP", PdfSettings.ImageQuality, PdfSettings.Dpi);
                break;
        }
        ...
        if (img != null)
            try
            {
                i = iTextSharp.text.Image.GetInstance(ImageToByteArray(img));
            }
            catch (Exception e)
            {
                Debug.WriteLine(e);
    ...
    public void SaveAndClose()
    {
        ...
        {
            Debug.WriteLine("Failed to close Document.");
        }
    }
